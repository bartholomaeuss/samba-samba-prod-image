...
provide_container(){
    ...
    ssh -v -l "${user}" "${remote}" "echo "UUID=${external_drive_1}  /mnt/external_drive_1  ${external_drive_1_format}  defaults  0  0" | sudo tee -a /etc/fstab"
    ssh -v -l "${user}" "${remote}" "echo "UUID=${external_drive_2}  /mnt/external_drive_2  ${external_drive_2_format}  defaults  0  0" | sudo tee -a /etc/fstab"
